...
arg_parser = argparse.ArgumentParser(description='Run InSpectreGadget in parallel')
...
arg_parser.add_argument('-c', '--config', type=str, required=True, help='Which configuration file to use')
arg_parser.add_argument('-o', '--out', type=str, required=True, help='Where to store all the output')
arg_parser.add_argument('-t', '--timeout', type=int, required=True, help='Max seconds per entrypoint')
arg_parser.add_argument('-j', '--jobs', type=int, required=True, help='How many parallel jobs')
...
print(f"Running inspectre analyze {binary} with {jobs} parallel jobs (output will be saved in {out_folder})")
